...
`recommend_for_mood`, `recommend_from_playlist_for_mood` and `read_my_mood` are **YouTube-only and refuse on other backends with an explanatory error**, rather than silently returning YouTube ids as another service's recommendations (which is what they did before this was enforced). The *index* half of that limit is gone — the graph atlas gives a Spotify library 40.2% mood coverage, measured 2026-08-29, against YouTube's editorial atlas at the same 40% — but the mood pipeline itself is still graph-blind (`recommend.build` gathers candidates without the graph), so on a backend with no native signals it returns nothing at all. The gate stays shut until that is fixed rather than opening onto an empty result; see `PLAN.md`, "The Spotify mood gate, measured". `recommend_from_song`, `recommend_from_playlist`, `songs_by_artist`, `refresh_library`, `record_feedback`, `explain_recommendation` and `index_status` are available on both backends.
